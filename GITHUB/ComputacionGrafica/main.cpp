...
protected:
   ...
   bool xIzq, xDer, xIzqObj, xDerObj, yIzq, yDer;
   float xPos, xPosObj, yPos;
...
public:
	...
	void initialize_textures(void)
	{
		...
		FIBITMAP* bitmap = FreeImage_Load(
			FreeImage_GetFileType("./Mallas/trunk.jpg", 0),
			"./Mallas/trunk.jpg");  //*** Para Textura: esta es la ruta en donde se encuentra la textura
		...
		glTexImage2D(GL_TEXTURE_2D, 0, internal_mode, nWidth, nHeight,
			0, pixel_mode, GL_UNSIGNED_BYTE, (void*)FreeImage_GetBits(pImage));
	...
	virtual void OnInit()
	{
	 ...
	 //*** Para Textura: abre los shaders para texturas
	  ...
	  if (!objmodel_ptr)
	  {
		  objmodel_ptr = glmReadOBJ("./Mallas/Proto.obj");
		  if (!objmodel_ptr)
			  exit(0);
	...
	if (!objmodel_ptr1)
	{
		objmodel_ptr1 = glmReadOBJ("./Mallas/palmtree.obj");
		if (!objmodel_ptr1)
			exit(0);
